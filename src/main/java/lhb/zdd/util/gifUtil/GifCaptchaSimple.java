...
/**
 * <p>
 * Gif验证码类,简单图片只能是静态图片，不支持动画
 * </p>
 *
 * @author: zhouxj
 * @version:1.01
 */
public class GifCaptchaSimple extends Captcha {
	char[] rands = { 49, 49, 49, 49 }; // 初始为1111

	public GifCaptchaSimple() {
	}

	public GifCaptchaSimple(int width, int height) {
		this.width = width;
		this.height = height;
	}

	public GifCaptchaSimple(int width, int height, int len) {
		this(width, height); // 宽度，高度
		this.len = len; // 验证码字符数量
	}

	public GifCaptchaSimple(int width, int height, int len, char[] str) {
	...
	public GifCaptchaSimple(int width, int height, int len, Font font) {
	...
	public void out(OutputStream os) {
		try {
			...
			//for (int i = 0; i < len; i++) {
				frame = graphicsImage(fontcolor, rands, 1);
...
//					getAlpha(Randoms.num(flag+1), Randoms.num(i+1))); //使用随机透明度
			...
			g2d.drawOval(Randoms.num(width), Randoms.num(height),
					5 + Randoms.num(10), 5 + Randoms.num(10)); //画椭圆
			g2d.drawRect(Randoms.num(width), Randoms.num(height),  Randoms.num(10), Randoms.num(20)); //画矩形
